...
formatter, Spexor: same *direction* (behaviour + status) or local-first specs.
Different *job* (post-run HTML, SaaS authoring, or manual execution). Borrow the
...
- Supporting Cucumber Messages NDJSON (or any new report format) unless ingest
  later *collapses* to fewer formats — not expands.
